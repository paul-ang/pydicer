...
def transform_point_set_from_dicom_struct(dicom_image, dicom_struct, spacing_override=None):
    ...
    for struct_index, struct_name in enumerate(struct_name_sequence):
        # Only convert GTV or first struc for compatibility
        if struct_index != 0 and 'gtv' not in struct_name.lower() and 'ganglion' not in struct_name.lower() and 'ig2' not in struct_name.lower() and 'iig2' not in struct_name.lower() and 'g3' not in struct_name.lower():
            continue
